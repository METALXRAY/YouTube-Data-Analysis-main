...
client = pm.MongoClient('mongodb+srv://papabearmetalxray:bTTRicLFoLciguy8@cluster0.txxr32k.mongodb.net/?retryWrites=true&w=majority')
db = client["youtube_project"]
collection = db["YouTube"]

# Sql Python Connectivity
metalxray = pg2.connect(host='localhost', user='postgres', password='1234', database='youtube')
cursor = metalxray.cursor()

# API Key :

api_key = 'AIzaSyAvEWjFxGWiw_eONHy4DqPLqU3dtksLRfc' # provide your api key here
...
#-------------------------------------------------------------------------------------------------------------
    ...
    def doc2df(self, Channel_name):
        ...
        try:
            ...
            comment_data = comment_data.reindex(columns=['Comment_Id','Comment_Author', 'Comment_Text', 'Comment_PublishedAt', "Video_id"])
            # print(comment_data)
        except :
            randcomment=random.randint(1, 999999)
            fullcomment_data = [{'Comment_id':randcomment,'Comment_Author': 'Disabled', 'Comment_Text': 'Disabled','Comment_PublishedAt':"2023-07-29T12:00:32Z", 'Video_Id': None }]
            comment_data = pd.DataFrame(fullcomment_data)
            #print(comment_data)

        # print("THIS IS COMMENT DATA !!!!!!!!",comment_data)
...
#--------------------------------------------------------------------------------------------------------
    ...
    def datatransform(self, cd, pl, vd, cod):
        ...
        video['Published_date'] = video['Published_date'].apply(lambda x: str(x))
        ...
        video['year_Published_At'] = pd.to_datetime(video['Published_date']).dt.year

        # Comment Data Transformation
        comment["Comment_PublishedAt"] = pd.to_datetime(comment["Comment_PublishedAt"])

        comment['Comment_PublishedAt'] = comment['Comment_PublishedAt'].apply(lambda x: str(x))

        comment['year_Published_At'] = pd.to_datetime(comment["Comment_PublishedAt"]).dt.year
    ...
    def da_query(self):
        ...
        if Choice == "Given Questions":
                ...
                # 3
                    ...
                    if st.button("GET SOLUTION"):
                        query_3 = "select channel_name  , title   from video order by ViewCount desc limit 10"
                ...
                # 4
                    ...
                    if st.button("GET SOLUTION"):
                        query_4 = "select title ,CommentCount from video  order by CommentCount desc"
                ...
                # 5
                    ...
                    if st.button("GET SOLUTION"):
                        query_5 = "select channel_name , title  from video order by likecount desc limit 10"
                ...
                # 6
                    ...
                    if st.button("GET SOLUTION"):
                        query_6 = "select title  , likecount , dislikecount  from video  order by likecount desc "
                ...
                # 8
                    ...
                    if st.button("GET SOLUTION"):
                        query_8 = "select distinct(channel_name) , year_Published_At   from video where year_Published_At = 2022 order by channel_name "
                ...
                # 10
                    ...
                    if st.button("GET SOLUTION"):
                        query_10 = "select channel_name  , title from video order by CommentCount desc limit 100"
...
st.title(":red[Youtube] Data Harvesting and Warehousing Project by- Aaryan Bhatia Ghosh")
...
# Stage 2 : Documents Names  Selection process to Migration of mongo Docs into Sql records
    ...
    if len(Names)>0:
        ...
        if Channel_name not in sql_chan_names:
            if Channel_name in Names:
                ...
                final_vd =f_vd.reindex(columns=['Video_Id','Playlist_Id','channel_id','Channel_name','Title','Published_date','Description','ViewCount','LikeCount','FavoriteCount','CommentCount','Duration','DislikeCount','year_Published_At'])
    ...
    st.subheader("For More Queries Connect :green[Aaryan Bhatia Ghosh] Through [Linkedln](https://www.linkedin.com/in/aaryanbhatiaghosh/)")
    st.subheader("Make sure to give ⭐ In [GitHub](https://github.com/METALXRAY) ")
